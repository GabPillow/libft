...
/*   Updated: 2019/03/22 16:26:08 by grochefo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_print_d(char *buff, va_list ap, t_format *form)
{
	...
	ft_exit(((buff = ft_strjoinplus("+", buff, 2)) != NULL)) : 0;
...
char	*ft_print_f(char *buff, va_list ap, t_format *form)
{
	...
	ft_exit(((buff = ft_strjoinplus(buff, ".", 1)) != NULL)) : 0;
	(int)(form->large - ft_strlen(buff)) > 0 ?
	buff = ft_manage_space(buff, form) : 0;
	return (buff);
}
...
char	*ft_print_u(char *buff, va_list ap, t_format *form)
{
	...
	form->flag & H ? buff = ft_uitoa((short)va_arg(ap, int)) : 0;
	form->flag & HH ? buff = ft_uitoa((char)va_arg(ap, int)) : 0;
